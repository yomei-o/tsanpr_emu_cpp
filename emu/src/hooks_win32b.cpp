...
void Emulator::install_win32_extra_hooks() {
    ...
    // On a Windows host the version is the host's - and specifically the one a
    // plain unmanifested process there is told, since that is what the guest
    // would have seen had it run natively.  A guest that folds this into an
    // identity of the machine gets a different answer otherwise.
    auto os_version = [](Emulator& e, bool wide) {
        uint32_t major = 10, minor = 0, build = 19045, platform = 2;
#if defined(_WIN32)
        OSVERSIONINFOA v;
        v.dwOSVersionInfoSize = sizeof v;
        if (GetVersionExA(&v)) {
            major = static_cast<uint32_t>(v.dwMajorVersion);
            minor = static_cast<uint32_t>(v.dwMinorVersion);
            build = static_cast<uint32_t>(v.dwBuildNumber);
            platform = static_cast<uint32_t>(v.dwPlatformId);
        }
#endif
        uint64_t p = e.arg_slot(0);
        if (p) {
            e.mem.write32(p + 4, major);
            e.mem.write32(p + 8, minor);
            e.mem.write32(p + 12, build);
            e.mem.write32(p + 16, platform);
            // szCSDVersion is a fixed array in the structure, and a caller reads
            // it whether or not anything was put there.
            e.mem.write8(p + 20, 0);
            if (wide) e.mem.write8(p + 21, 0);
        }
        e.set_result(1);
    };
    win32("GetVersionExA", 1, [os_version](Emulator& e) { os_version(e, false); });
    win32("GetVersionExW", 1, [os_version](Emulator& e) { os_version(e, true); });
