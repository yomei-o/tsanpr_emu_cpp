...
static const bool g_watch_initialized = [] {
    ...
    if (const char* w = std::getenv("X86EMU_RWATCH")) {
        g_rwatch_addr = std::strtoull(w, nullptr, 16);
        g_rwatch_on = g_rwatch_addr != 0;
    }
    return true;
}();
...
void watch_report_read(uint64_t addr, uint64_t len) {
    std::fprintf(stderr, "[rwatch] read %llu bytes at %llX rip=%llX\n",
                 (unsigned long long)len, (unsigned long long)addr,
                 (unsigned long long)g_watch_rip);
}
