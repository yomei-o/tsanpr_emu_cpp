...
bool Cpu::execute_sse(uint8_t op) {
    ...
    switch (op) {
        ...
        case 0x7D:
        case 0xD0: {
            // SSE3 horizontal add/sub (0x7C/0x7D) and add/subtract (0xD0).
            // These are advertised in CPUID (SSE3), so they must exist.  The
            // prefix selects width the SSE3 way: 66 -> packed double, F2 ->
            // packed single (the reverse of the scalar forms).
            const bool is_double = sel == Sel::P66;
            if (sel != Sel::P66 && sel != Sel::PF2) return false;
            RM rm = decode_modrm();
            Xmm dst = xmm[modrm_reg_], src = xmm_read(rm), r{};
            if (op == 0x7C) {  // HADD
                if (is_double) { r.f64[0] = dst.f64[0] + dst.f64[1];
                                 r.f64[1] = src.f64[0] + src.f64[1]; }
                else { r.f32[0] = dst.f32[0] + dst.f32[1]; r.f32[1] = dst.f32[2] + dst.f32[3];
                       r.f32[2] = src.f32[0] + src.f32[1]; r.f32[3] = src.f32[2] + src.f32[3]; }
            } else if (op == 0x7D) {  // HSUB
                if (is_double) { r.f64[0] = dst.f64[0] - dst.f64[1];
                                 r.f64[1] = src.f64[0] - src.f64[1]; }
                else { r.f32[0] = dst.f32[0] - dst.f32[1]; r.f32[1] = dst.f32[2] - dst.f32[3];
                       r.f32[2] = src.f32[0] - src.f32[1]; r.f32[3] = src.f32[2] - src.f32[3]; }
            } else {  // 0xD0 ADDSUB: even lanes subtract, odd lanes add
                if (is_double) { r.f64[0] = dst.f64[0] - src.f64[0];
                                 r.f64[1] = dst.f64[1] + src.f64[1]; }
                else { r.f32[0] = dst.f32[0] - src.f32[0]; r.f32[1] = dst.f32[1] + src.f32[1];
                       r.f32[2] = dst.f32[2] - src.f32[2]; r.f32[3] = dst.f32[3] + src.f32[3]; }
            }
            xmm[modrm_reg_] = r;
            return true;
        }
