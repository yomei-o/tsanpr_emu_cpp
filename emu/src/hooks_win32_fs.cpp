...
void Emulator::install_win32_fs_hooks() {
    ...
    // Which key a guest asked for is the whole story when it goes on to report
    // something of its own missing, so the calls that name one say what it was.
    // These are registered first; add_hook keeps the earliest registration, so
    // the generic loop below leaves them alone.
    auto reg_name = [](Emulator& e, uint64_t p, bool wide) {
        if (!p) return std::string("(null)");
        return wide ? utf16_to_utf8(e, p, -1) : e.mem.read_cstring(p);
    };
    add_hook("RegOpenKeyExW", 0, [kFileNotFound, reg_name](Emulator& e) {
        e.log_call("RegOpenKeyEx(root=0x%llX, %s)",
                   static_cast<unsigned long long>(e.arg_slot(0)),
                   reg_name(e, e.arg_slot(1), true).c_str());
        e.set_result(kFileNotFound);
    });
    add_hook("RegOpenKeyExA", 0, [kFileNotFound, reg_name](Emulator& e) {
        e.log_call("RegOpenKeyEx(root=0x%llX, %s)",
                   static_cast<unsigned long long>(e.arg_slot(0)),
                   reg_name(e, e.arg_slot(1), false).c_str());
        e.set_result(kFileNotFound);
    });
    add_hook("RegQueryValueExW", 0, [kFileNotFound, reg_name](Emulator& e) {
        e.log_call("RegQueryValueEx(%s)", reg_name(e, e.arg_slot(1), true).c_str());
        e.set_result(kFileNotFound);
    });
    add_hook("RegQueryValueExA", 0, [kFileNotFound, reg_name](Emulator& e) {
        e.log_call("RegQueryValueEx(%s)", reg_name(e, e.arg_slot(1), false).c_str());
        e.set_result(kFileNotFound);
    });
