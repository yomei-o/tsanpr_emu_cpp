...
// A path as Windows spells it: absolute, with backslashes.  A runtime locates
// its own installation from one of these, so a relative path or the wrong
// separator sends it looking in the wrong place.
std::string windows_form(std::string path) {
    bool absolute = (path.size() > 1 && path[1] == ':') ||
                    (!path.empty() && (path[0] == '/' || path[0] == '\\'));
    if (!absolute) {
#if defined(_WIN32)
        wchar_t cwd[4096];
        if (_wgetcwd(cwd, 4096))
            path = utf16_string_to_utf8(std::u16string(cwd, cwd + wcslen(cwd))) + "/" + path;
#else
        char cwd[4096];
    ...
    // Windows hands back a canonical path, with "." and ".." already resolved -
    // a guest started as ".\prog.exe" is told where it actually is.  Leaving
    // them in is visible to anything that compares paths or folds one into an
    // identity, and that is a difference which surfaces a long way from here.
    std::string prefix;
    size_t at = 0;
    ...
    while (at <= path.size()) {
        ...
        std::string part = path.substr(at, sep - at);
        if (part == "..") {
...
std::string program_path(const Emulator& e) {
    return windows_form(e.args().empty() ? "program.exe" : e.args()[0]);
}
...
void write_system_info(Emulator& e, uint64_t p) {
    ...
    // How many processors the machine has is a fact about the machine, not about
    // the emulator's scheduler: a guest sizing a thread pool from it gets green
    // threads either way, and a guest folding it into an identity of the machine
    // needs the real number.
    uint32_t processors = 1;
#if defined(_WIN32)
    SYSTEM_INFO host{};
    GetSystemInfo(&host);
    if (host.dwNumberOfProcessors) processors = host.dwNumberOfProcessors;
#else
    if (unsigned n = std::thread::hardware_concurrency()) processors = n;
#endif
    uint64_t mask = processors >= 64 ? ~0ull : ((1ull << processors) - 1);
    // wProcessorLevel/Revision (and the processor type) are facts about the
    // machine, exactly like the processor count: a guest folding them into an
    // identity of the machine needs the host's, not a hard-coded default.
    uint32_t proc_type = w64 ? 8664 : 586;
    uint16_t proc_level = 6;
    uint16_t proc_revision = 0x3A09;
#if defined(_WIN32)
    if (host.dwProcessorType) proc_type = host.dwProcessorType;
    proc_level = host.wProcessorLevel;
    proc_revision = host.wProcessorRevision;
    if (host.dwActiveProcessorMask) mask = host.dwActiveProcessorMask;
#endif
    e.mem.write_sized(p + 8 + 2 * ps, ps, mask);  // dwActiveProcessorMask
    uint64_t after_mask = p + 8 + 3 * ps;
    e.mem.write32(after_mask, processors);   // dwNumberOfProcessors
    e.mem.write32(after_mask + 4, proc_type);  // dwProcessorType
    e.mem.write32(after_mask + 8, 0x10000);  // dwAllocationGranularity
    e.mem.write16(after_mask + 12, proc_level);       // wProcessorLevel
    e.mem.write16(after_mask + 14, proc_revision);    // wProcessorRevision
}
...
void Emulator::install_win32_hooks() {
    ...
    // A name must resolve the way GetModuleHandleEx resolves it: cl 14.31's CRT
    // asks for the synch apiset and then kernel32 while wiring up WaitOnAddress,
    // and fastfails if *both* come back null - answering 0 for every name was
    // an instant INT3 ten thousand instructions in.
    auto get_module_handle = [](Emulator& e, bool wide) {
        uint64_t name_ptr = e.arg_slot(0);
        if (!name_ptr) {
            e.set_result(e.image().image_base);
            return;
        }
        std::string name = wide ? utf16_to_utf8(e, name_ptr, -1)
                                : e.mem.read_cstring(name_ptr);
        uint64_t base = 0;
    ...
    win32("GetModuleHandleA", 1,
          [get_module_handle](Emulator& e) { get_module_handle(e, false); });
    win32("GetModuleHandleW", 1,
          [get_module_handle](Emulator& e) { get_module_handle(e, true); });
    ...
    // A null module means the program; anything else is a loaded DLL asking
    // where *it* lives.  Answering the program's path either way tells a DLL it
    // sits in a directory it does not, which is how a library that loads its own
    // data files from beside itself ends up finding none of them.
    auto module_file_name = [](Emulator& e) {
        uint64_t module = e.arg_slot(0);
        if (module) {
            if (Emulator::Module* m = e.module_for(module))
                return windows_form(m->path);
        }
        return program_path(e);
    };
    win32("GetModuleFileNameA", 3, [module_file_name](Emulator& e) {
        std::string name = module_file_name(e);
    ...
    win32("GetModuleFileNameW", 3, [module_file_name](Emulator& e) {
        std::string name = module_file_name(e);
    ...
    win32("GetProcAddress", 2, [](Emulator& e) {
        ...
        if (e.options().trace_calls)
            e.log_call("GetProcAddress(0x%llX, %s) = 0x%llX",
                       (unsigned long long)module, symbol.c_str(),
    ...
    win32("GetACP", 0, [](Emulator& e) {
        const char* v = std::getenv("EMU_ACP");
        e.set_result(v ? (uint64_t)std::strtoul(v, nullptr, 10) : 65001);  // UTF-8
    });
    win32("GetOEMCP", 0, [](Emulator& e) {
        const char* v = std::getenv("EMU_ACP");
        e.set_result(v ? (uint64_t)std::strtoul(v, nullptr, 10) : 437);
    });
...
void Emulator::install_ucrt_hooks() {
    ...
    ucrt("__stdio_common_vsprintf", [](Emulator& e) {
        ...
        Args tail = Args::va_list_at(e, va);
        std::string s = format_guest(e, fmt, tail);
        // What a guest formats into a buffer is often the most direct statement
        // of what it is about to do with it, and none of it reaches stdout.
        e.log_call("sprintf -> \"%s\"", s.c_str());
    ...
    ucrt("__stdio_common_vswprintf", [](Emulator& e) {
        ...
        uint64_t va = a.next_ptr();
        Args tail = Args::va_list_at(e, va);
        std::string narrow = format_guest(e, fmt, tail, true);
        e.log_call("swprintf -> \"%s\"", narrow.c_str());
        std::u16string s = utf8_to_utf16(narrow);
        bool truncated = count && s.size() >= count;
        if (buf && count) {
            size_t n = truncated ? static_cast<size_t>(count - 1) : s.size();
