...
// hooks_winsock.cpp: the Winsock 1.1 ordinal table, which an import by ordinal
// needs before any hook can be found by name.
...
uint64_t Emulator::load_library(const std::string& raw_name) {
    ...
    // The lookup/dedup key is lowercased (Windows matches module names case-
    // insensitively), but the *path* keeps its original case: GetModuleFileName
    // is case-preserving, and a guest that folds its own module path into a
    // machine fingerprint needs the exact spelling the OS would hand back.
    std::string orig = raw_name;
    if (name.find('.') == std::string::npos) { name += ".dll"; orig += ".dll"; }
    ...
    std::string path = find_library_file(orig);
    if (path.empty()) {
...
const char* Emulator::well_known_ordinal(const std::string& dll, uint32_t ordinal) {
    ...
    if (lower == "ws2_32.dll" || lower == "ws2_32" || lower == "wsock32.dll")
        return winsock_ordinal_name(ordinal);
    return nullptr;
}
