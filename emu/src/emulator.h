...
// guest opened with CreateFile) so that a guest handle can be routed back to the
// host object it stands for.  Tokens are handed out above the fd-handle range and
// never collide with it.  Defined in hooks_files.cpp; used by the file and HID
// hooks.  On a non-Windows host these still compile but are simply unused.
...
public:
    struct Options {
        ...
        std::vector<uint32_t> dialog_commands;
    ...
    void install_win32_dll_hooks();
    void install_winsock_hooks();
    void install_gui_hooks();
    void install_wmi_hooks();
    void install_registry_hooks();
    // Built by install_wmi_hooks(): makes an IWbemLocator in guest memory.
    // Empty when WMI is not available, which is how CoCreateInstance decides
    // between handing back an object and reporting no such class.
    std::function<uint64_t(Emulator&)> wmi_create_locator_;
