...
void Emulator::install_cl_hooks() {
    ...
    win32("CryptGenRandom", 3, [](Emulator& e) {
        // Deterministic, but a *stream*: resetting the state per call handed
        // out identical bytes every time, and cl 14.31 retries its random
        // temporary-file name against a collision by drawing again - seven
        // identical draws and it gives up with D8037.
    ...
    // diagnostic anywhere.
    {
        ...
        auto feed = [hashes](Emulator& e, uint64_t handle, uint64_t buf, uint64_t len) -> bool {
            ...
            if (std::getenv("EMU_HASHLOG")) {
                std::fprintf(stderr, "[hash] handle=%llx feed %llu byte(s): ascii=\"",
                             (unsigned long long)handle, (unsigned long long)len);
                for (uint64_t i = 0; i < len && i < 400; ++i) {
                    unsigned c = (unsigned)e.mem.read_sized(buf + i, 1);
                    std::fprintf(stderr, "%c", (c >= 32 && c < 127) ? (int)c : (c == 0 ? '#' : '.'));
                }
                std::fprintf(stderr, "\" hex=");
                for (uint64_t i = 0; i < len && i < 64; ++i)
                    std::fprintf(stderr, "%02llx", (unsigned long long)e.mem.read_sized(buf + i, 1));
                std::fprintf(stderr, "\n");
            }
            return true;
        };
    ...
    win32("RtlCreateUnicodeString", 2, [](Emulator& e) {
        ...
        // UNICODE_STRING.Buffer follows the two lengths at pointer alignment.
        e.mem.write_sized(out + (e.is64() ? 8 : 4), e.pointer_size(), buf);
        e.set_result(1);
    });
    ...
    auto nt_path = [](Emulator& e, uint64_t object_attributes) -> std::string {
        ...
        // OBJECT_ATTRIBUTES and UNICODE_STRING both change shape with the
        // pointer size: ObjectName sits at +0x10/+0x8 and the string buffer at
        // +8/+4.  Reading the 64-bit offsets in a 32-bit guest produced an
        // empty path, and a c1 that "could not find" a directory that was
        // plainly there.
        uint64_t name = e.mem.read_sized(object_attributes + (ps == 8 ? 0x10 : 0x8), ps);
        if (!name) return {};
        uint32_t bytes = e.mem.read16(name);
        uint64_t buf = e.mem.read_sized(name + (ps == 8 ? 8 : 4), ps);
        if (!buf) return {};
        ...
        uint64_t root = e.mem.read_sized(object_attributes + (ps == 8 ? 8 : 4), ps);
        if (root) {
        ...
        auto map_view = [maps, views](Emulator& e, uint64_t base) {
            ...
            if (e.files.seek(m.fd, static_cast<int64_t>(offset), 0) >= 0) {
                ...
                if (e.options().trace_calls)
                    e.log_call("MapViewOfFile(fd %d, offset 0x%llX, size 0x%llX) read %lld,"
                               " head %02X %02X %02X %02X",
                               m.fd, (unsigned long long)offset, (unsigned long long)size,
                               (long long)got, buf.size() > 0 ? buf[0] : 0,
                               buf.size() > 1 ? buf[1] : 0, buf.size() > 2 ? buf[2] : 0,
                               buf.size() > 3 ? buf[3] : 0);
            }
    ...
    ucrt("_waccess_s", [](Emulator& e) {
        ...
        int r = FileTable::stat_path(path, st) == 0 ? 0 : 2;
        if (e.options().trace_calls)
            std::fprintf(stderr, "[hook] _waccess_s(%s, %d) = %d\n", path.c_str(),
                         static_cast<int>(e.arg_slot(1)), r);
    ...
    ucrt("_wfullpath", [](Emulator& e) {
        ...
        if (!absolute) {
            ...
            if (_wgetcwd(cwd, 4096))
                full = utf16_string_to_utf8(std::u16string(cwd, cwd + wcslen(cwd))) + "\\" + in;
#else
            char cwd[4096];
    ...
    auto wcs_copy = [](Emulator& e, bool cat, bool bounded) {
        ...
        // _TRUNCATE is (size_t)-1, which is 32 bits wide in a 32-bit guest:
        // comparing against ~0ull there turns every truncating copy into a
        // "bounded copy that does not fit", which empties the destination -
        // and an emptied archive member name is LNK1127, library corrupt.
        const uint64_t kTruncate = e.is64() ? ~0ull : 0xFFFFFFFFull;
