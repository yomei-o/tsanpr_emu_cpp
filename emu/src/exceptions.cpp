...
void Emulator::install_exception_hooks() {
    ...
    win32("RaiseException", [](Emulator& e) {
        ...
        // Decode an MSVC C++ throw (0xE06D7363) to its type name: params are
        // {EH magic, thrown object, _ThrowInfo, image base}, and the ThrowInfo
        // chains ThrowInfo -> CatchableTypeArray -> CatchableType -> TypeDescriptor,
        // whose name field is the mangled type.  Names the exception a licensing
        // path throws, which a bare exception code does not.
        if (code == 0xE06D7363u && params.size() >= 4 && params[0] == 0x19930520u) {
            uint64_t obj = params[1], ti = params[2], base = params[3];
            uint32_t cta_rva = e.mem.read32(ti + 0xC);
            if (cta_rva) {
                uint64_t cta = base + cta_rva;
                int32_t ntypes = static_cast<int32_t>(e.mem.read32(cta));
                uint32_t ct_rva = e.mem.read32(cta + 4);
                uint64_t ct = base + ct_rva;
                uint32_t td_rva = e.mem.read32(ct + 4);
                uint64_t td = base + td_rva;
                std::string name;
                for (int i = 0; i < 200; ++i) {
                    unsigned c = static_cast<unsigned>(e.mem.read_sized(td + 0x10 + i, 1));
                    if (!c) break;
                    name.push_back(static_cast<char>(c));
                }
                std::string msg;  // std::exception-style: object+0x?? often holds char*
                for (int off = 0; off <= 0x18 && msg.empty(); off += 8) {
                    uint64_t p = e.mem.read64(obj + off);
                    if (p > 0x10000 && p < 0x7fffffffffffull) {
                        std::string s;
                        for (int i = 0; i < 160; ++i) {
                            unsigned c = static_cast<unsigned>(e.mem.read_sized(p + i, 1));
                            if (!c) break;
                            if (c < 32 || c >= 127) { s.clear(); break; }
                            s.push_back(static_cast<char>(c));
                        }
                        if (s.size() >= 4) msg = s;
                    }
                }
                std::fprintf(stderr, "[c++ throw] types=%d type=\"%s\" msg=\"%s\"\n",
                             ntypes, name.c_str(), msg.c_str());
                if (std::getenv("EMU_THROW_TRACE")) {
                    std::string tr = e.stack_trace();
                    std::fprintf(stderr, "[c++ throw] caller chain:\n%s", tr.c_str());
                }
            }
        }
