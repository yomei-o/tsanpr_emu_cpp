...
public:
    ...
    uint8_t read8(uint64_t addr) const {
        if (g_rwatch_on && addr == g_rwatch_addr) watch_report_read(addr, 1);
        return *host_ptr(addr, false);
    }
...
private:
    ...
    T read_int(uint64_t addr) const {
        if (g_rwatch_on && addr <= g_rwatch_addr && g_rwatch_addr < addr + sizeof(T))
            watch_report_read(addr, sizeof(T));
